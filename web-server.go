...
const (
	...
	gopresentCertFile   = "/etc/letsencrypt/live/gopresent.io/fullchain.pem"
	gopresentKeyFile    = "/etc/letsencrypt/live/gopresent.io/privkey.pem"
	painedecasaCertFile = "/etc/letsencrypt/live/painedecasa.ro/fullchain.pem"
	painedecasaKeyFile  = "/etc/letsencrypt/live/painedecasa.ro/privkey.pem"
)
...
func redirect(w http.ResponseWriter, r *http.Request) {
	switch r.Host {
	case "www.birdnest.io", "birdnest.io":
		http.Redirect(w, r, "https://birdnest.io"+r.RequestURI, http.StatusSeeOther)
	case "www.gopresent.io", "gopresent.io":
		http.Redirect(w, r, "https://gopresent.io"+r.RequestURI, http.StatusSeeOther)
	case "www.painedecasa.ro", "painedecasa.ro":
		http.Redirect(w, r, "https://painedecasa.ro"+r.RequestURI, http.StatusSeeOther)
	default:
...
type mux struct {
	...
	gopresent      http.Handler
	painedecasa    http.Handler
}
...
func (m *mux) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case "www.gopresent.io", "gopresent.io":
		m.gopresent.ServeHTTP(w, r)
	case "www.painedecasa.ro", "painedecasa.ro":
		m.painedecasa.ServeHTTP(w, r)
	default:
...
func newMux() http.Handler {
	...
	return &mux{
		gbbr:        gbbr,
		birdnest:    bird,
		gopresent:   gopresent.NewApp(gopresent.Options{}),
		painedecasa: http.FileServer(http.Dir("/home/www/painedecasa/public")),
	}
}
...
func main() {
	...
	gopresentCert, err := tls.LoadX509KeyPair(gopresentCertFile, gopresentKeyFile)
	if err != nil {
		log.Fatal(err)
	}
	painedecasaCert, err := tls.LoadX509KeyPair(painedecasaCertFile, painedecasaKeyFile)
	if err != nil {
		log.Fatal(err)
	}

	tlsConfig := &tls.Config{Certificates: []tls.Certificate{gbbrCert, birdnestCert, gopresentCert, painedecasaCert}}
